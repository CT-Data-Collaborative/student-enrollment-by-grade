...
##################################################################
#
# Processing Script for Student-Enrollment-by-Grade
...
cols_to_stack <- c("Pre-Kindergarten", "Kindergarten", "Grade 1", "Grade 2", 
...
test <- complete_student_enrollment_long[,c("District", "Year", "Grade")]
test2<-test[duplicated(test), ]

complete_student_enrollment_long <- drop_na(complete_student_enrollment_long, 'District')

#Write CSV
write.table(
  complete_student_enrollment_long,
  file.path(path_to_top_level, "data", "student_enrollment_by_grade_2008-2021.csv"),
  sep = ",",
  row.names = F
)
